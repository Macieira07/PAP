...
if (isset($_GET['modal'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $tipo = $_POST['tipo'];
        $descricao = $_POST['descricao_hidden'];
        $data_inicio = $_POST['data_inicio'];
        $data_fim = $_POST['data_fim'];
        $custo = $_POST['custo'];
        $pago = isset($_POST['pago']) ? 1 : 0;
        
        $stmt = $conexao->prepare("INSERT INTO manutencao (M_id_casa, M_tipo, M_descricao, M_data_inicio, M_data_fim, M_custo, M_pago) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssdi", $id_casa, $tipo, $descricao, $data_inicio, $data_fim, $custo, $pago);
        $stmt->execute();
    ...
    ?>
    <div>
        <h2 style="margin-top:0; margin-bottom:18px; text-align:center; color:#2e5090;">Adicionar Manutenção</h2>
        <form method="post" id="formWizardManutencao" style="display:flex; flex-direction:column; gap:10px;" onsubmit="return sincronizarDescricao()">
            <!-- Etapa 1 -->
            <div class="wizard-step" id="wizardStep1">
                ...
                <label style="display:flex; flex-direction:column; gap:4px;">Descrição:
                    <textarea id="descricao" name="descricao" readonly rows="4" style="resize:vertical; min-height:60px; max-width:100%;"></textarea>
...
document.addEventListener('DOMContentLoaded', function() {
  atualizarDescricao();
  document.getElementById('tipo').addEventListener('change', atualizarDescricao);
});


    function sincronizarDescricao() {
        // Garante que o campo hidden tem o valor correto antes de submeter
        document.getElementById('descricao_hidden').value = document.getElementById('descricao').value;
        return true;
    }
